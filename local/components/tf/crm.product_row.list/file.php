    ...
    //if($groupid>0){
        ...
        foreach($sheetNames as $sheetName) {
            ...
            for ($i=1;$i<count($data);$i++){
                ...
                $new=[
                    "ID" => 0,
                    "PRODUCT_NAME" => trim($table[0]),
                    "PRODUCT_ID" => 0,
                    "QUANTITY" => $checkTime->getQuantity($resultTime["DATE_START"]["VALUE"],$resultTime["DATE_FINISH"]["VALUE"]),
                    "MEASURE_CODE" => 796,
                    "MEASURE_NAME" => "шт",
                    "PRICE" => trim($table[7]),
                    "PRICE_EXCLUSIVE" => trim($table[7]),
                    "ROLE_PROJECT" => $RoleProject->getIdByName(trim($table[1])),
                    "PRODUCT" => $Product->getIdByName(trim($table[2])),
                    "MODEL" => $Model->getIdByName(trim($table[3])),
                    "GRADE" => $Grade->getIdByName(trim($table[4])),
                    "YEARS_EXPERIENCE" => $YearsExperience->getIdByName(trim($table[5])),
                    "PREFERRED_LOCATION" => $PreferredLocation->getIdByName(trim($table[6])),
                    "AMOUNT_OVERHEAD" => trim($table[8]),
